...
fn dsp_gain_mix_algebra() {
    ...
    graph2.add_edge(auxide::graph::Edge {
        from_node: osc2,
        from_port: PortId(0),
        to_node: gain2,
        to_port: PortId(0),
    ...
    graph3.add_edge(auxide::graph::Edge {
        from_node: osc3,
        from_port: PortId(0),
        to_node: sink3,
        to_port: PortId(0),
    ...
    graph4.add_edge(auxide::graph::Edge {
        from_node: osc4a,
        from_port: PortId(0),
        to_node: mix,
        to_port: PortId(0),
    ...
    // Sum of two oscs
    for i in 0..64 {
        assert!((output4[i] - 2.0 * output3[i]).abs() < 0.01, "Mix should sum inputs");
    }
}