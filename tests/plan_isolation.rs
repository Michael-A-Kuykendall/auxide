...
fn plan_isolation() {
    ...
    let plan_b = Plan::compile(&graph_a, 64).unwrap();

    // Assert plan A remains unchanged
    let plan_a_again = Plan::compile(&graph_a, 64).unwrap(); // Recompile original? Wait, graph_a is mutated.
    ...
    let plan_b = Plan::compile(&graph_mutated, 64).unwrap();
    ...
    let mut runtime_a = Runtime::new(plan_a, &graph_original);
    let mut out = vec![0.0; 64];
    runtime_a.process_block(&mut out);
